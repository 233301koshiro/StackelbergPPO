...
class StairhardEnv(MujocoEnv, utils.EzPickle):

    HFIELD_FNAME = 'stair_terrain.png'
    ...
    def __init__(self, cfg, agent):
        ...
        if not osp.exists(self.hfield_file):           
            ...
            local_file_path = os.path.join(cfg.project_path, "assets", "mujoco_terrains", f"{self.HFIELD_FNAME}")
            shutil.copyfile(local_file_path, self.hfield_file)

        self.cfg = cfg
        self.env_specs = cfg.env_specs
        self.agent = agent
        if self.cfg.xml_name == "default":
            self.model_xml_file = os.path.join(cfg.project_path, "assets", "mujoco_envs", "stair-hard.xml")
        else:
    ...
    def step(self, a):
        ...
        # execution stage
            ...
            termination = not (np.isfinite(s).all() and (height > min_height) and (abs(ang) < np.deg2rad(max_ang)))
    ...
    def get_sim_obs(self):
        ...
        for i, body in enumerate(self.robot.bodies):
            ...
            if i == 0:
                height= self.get_height_above_ground(self.data.qpos[0:3])
                obs_i = [np.array([height]), self.data.qpos[3:7], qvel[:6], np.zeros(2)]
            else:
    ...
    def get_height_above_ground(self, pos):
        x, y, z = pos
        
        ray_start = np.array([x, y, z + 10.0], dtype=np.float64)
        ray_dir = np.array([0.0, 0.0, -1.0], dtype=np.float64)
        group_filter = np.zeros(6, dtype=np.bool_)
        group_filter[1] = True
        dist, geom_id = self.sim.ray(ray_start, ray_dir, include_static_geoms=True, exclude_body=-1, group_filter=group_filter)
        
        if dist < 0 or geom_id is None:
            return 0.0
        
        ground_z = ray_start[2] - dist
        height = z - ground_z

        # name = self.sim.model.id2name(geom_id, mujoco_py.const.GEOM)
        
        if height < 0 and height > -1e-6:
            height = 0.0
        return float(height)                   
    ...
    def viewer_setup(self):
        ...
        self.viewer.cam.lookat[:3] = self.data.qpos[:3]
