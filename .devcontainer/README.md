...
> ⚠️ **2026-09-08 追記: コンテナの定義はこの repo に 2 つある。**
> こちら（経路B）は**未使用**で、実際に動いているのは
> [`docker/docker-compose.yml`](../docker/README.md)（経路A）である。
> **片方を直したらもう片方も見ること**（CLAUDE.md §4-2）。
>
> とくに `/root/.claude`: **こちらは名前付きボリュームで保持するが、経路A はしていない。**
> つまり**この定義の対策は発動していない**。作り直す前の退避が要る。
