...
> ⚠️ **`requirements.txt` は使わないこと（2026-09-08）。** 記載が実環境と食い違っており、
> そのまま入れると **torch が 2.0.1 に落ちて GPU（sm_120）が動かなくなる**。
> 実環境の版は [docs/リポジトリ説明/環境の再現.md](docs/リポジトリ説明/環境の再現.md) を参照。
...
全ドキュメントの索引は [`docs/目次.md`](docs/目次.md) を参照。主要なものを以下に示す。

| ファイル | 内容 |
|---------|------|
| `docs/リポジトリ説明/システム概要.md` | Stackelberg PPO の仕組み・学習指標の説明 |
| `docs/移行記録/choreonoid移行.md` | Choreonoid 移行の詳細（バグ修正・アーキテクチャ変遷）|
| `docs/研究応用/設計/トポロジー固定最適化.md` | 初期形状の指定方法・トポロジー固定で属性値のみ最適化する方法 |
| `docs/研究応用/設計/メッシュXMLパイプライン.md` | 3D メッシュ → MuJoCo XML 変換パイプライン設計（爆発問題の対策）|
| `docs/研究応用/設計/メッシュ分割.md` | メッシュ分割手法の比較（スケルトン抽出・凹面・VLM 等）|
| `docs/移行記録/移行レポート.md` | Choreonoid 移行の技術リファレンス（アーキテクチャ・バグ修正・起動方法）|
