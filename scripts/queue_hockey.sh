...
#
# 監視: 投入と同時に monitor_training.py を立てる（9-41 の見逃しを受けて）。
#   ホッケーは報酬の上限が 0.8 しかなく、コスト・ペナルティの較正を外すと
#   「動かないのが最適」に静かに収束する。**32 epoch 気づかなかった。**
#   `--cube-zero-epochs 12` で、パックが 12 epoch 動かなければ警告が出る。
