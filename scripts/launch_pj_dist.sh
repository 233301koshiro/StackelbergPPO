...
#
# 目的（第4章 4.3.3.3 限界4 を閉じ、co-design を使う必然性を強める）:
#   これまでの判別実験（4.3.3.1 / 4.3.3.2 / 4.3.3.3）は4形態すべて**同じ配分×スケール違い**で、
...
#
# 設定は `tripo_pj_mid`（4.3.3.3 の中間スケール）と **xml_name 以外すべて同一**にしてある。
#
