...
- 形態パラメータ（ギア比・総リーチ）は morph dump 由来で原本に無いため、
  ②台帳（第4章 表4.4）の確定値をこのファイル内に明示的に書き、出典をコメントで残す
...
def fig_task_separation():
    # ②台帳（第4章 表4.4）の確定値。morph dump 由来で①原本には無いため転記。
    # 値を直すときは第4章 表4.4 と 形態比較.md を同時に直すこと。
