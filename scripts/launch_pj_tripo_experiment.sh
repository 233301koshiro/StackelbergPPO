...
#
# 目的: 4.4.3のPJ実験（rrbotの短腕/長腕、Reach）と同じロジックを、
...
#   タスク: Reach（target_x=0.8、既存のtripo_v2c_reach・rrbot L1/PJと同一設定）
...
#   PJ_tripo_long:  高い（tripo_v2c_reachのbest -8.27〜rrbot PJ_longの-4.9程度）
#
# 実行前の注意（2026-07-29時点）:
#   現在GPU/CPUスロットは3本（tripo_v2b_reach・M1_lenonly_1000×2）で埋まっている。
...
# Reach タスク共通オーバーライド（tripo_v2c_reach・rrbot L1/PJ と同一設定）
