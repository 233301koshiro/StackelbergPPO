...
# ⚠️ 円盤直径が大きすぎると OBB の最長辺がボーン軸でなくなる。
# **破綻は D/L ≈ 1.41 から**（9-69 で実測）。下の 0.6 倍制限は 2.3 倍の余裕がある。
# ⚠️ **2026-09-10 訂正: 「hockey は 1.56 と際どい」は根拠にならない。**
#   1.56 は最長辺÷次辺の比だが、**比は破綻の検出器として成立していない**（9-69）。
#   同じ比が「正常」と「破綻」の両方で出る。守っていたのはこの 0.6 倍制限の方である。
TIP = {
