...
class Robot:
    ...
    def get_shortest_distances(self):
        ...
        # return distances
        num_nodes = len(self.bodies)
        return np.zeros((num_nodes, num_nodes), dtype=np.int32)
    ...
    def get_laplacian_position_encoding(self, pos_enc_dim=4):
        ...
        # if k + 1 >= num_nodes:
        #     # 节点太少，eigsh 无法使用，转用稠密方法
        #     vals, vecs = eigh(L_sym)
        #     vecs = vecs[:, 1:k+1]  # 仍然忽略第一个特征向量
        # else:
        #     vals, vecs = eigsh(L_sym, k=k+1, which='SM')
        #     vecs = vecs[:, 1:k+1]

        # position_encoding = vecs  # shape: N, k
        ...
        num_nodes = len(self.bodies)
        return np.zeros((num_nodes, pos_enc_dim), dtype=np.float32)
