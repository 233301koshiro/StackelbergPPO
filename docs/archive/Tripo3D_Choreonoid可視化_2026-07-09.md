...
> **参照している `skeleton_extract.py` は既に存在しない。** 技術的知見（Y-up→Z-up 変換・URDF の joint origin・リンクローカル STL・マゼンタ規約）はすべて [研究応用/メッシュXMLパイプライン.md](../研究応用/設計/メッシュXMLパイプライン.md) に統合済み
...
`docs/研究応用/設計/メッシュ分割.md` の **プロンプトA**（直接生成）または **プロンプトB**（スケッチ整形）を使う。
