...
> **状態（2026-09-01 夕）**: **A1 は手描き → M1 まで完了。M2 以降と残り4枚が未着手。**
> A1 の3段階（参照画・手描き・M1 出力）と実測は「[A1 の実走記録](#a1-の実走記録2026-09-01)」にある。
> **M1 は比を保存した**（4.8 → 4.6）が、M2 以降が保つかは未確認。
> 次にやるのは A1 を Tripo3D へ通し、B1 を同じところまで進めて総リーチを比べること。
...
| # | 描き方 | 参照画 |
|---|---|---|
| A1 | 定規を使って丁寧に、正面（真横）から | ![A1 参照画](../sketch/A1_ref.jpeg) |
| A2 | フリーハンドで速く、正面から | ![A2 参照画](../sketch/A2_ref.jpeg) |
| A3 | やや斜め（3/4 アングル）から、リンクの太さを描き分けて | ![A3 参照画](../sketch/A3_ref.jpeg) |
...
| # | 形 | 狙い | 参照画 |
|---|---|---|---|
| B1 | **短くずんぐり**（= 既存 `short` 相当） | 第1層が学習ゼロ・数秒で「届きません、N 倍に伸ばしてください」と返すはず。**発表での実演に最適** | ![B1 参照画](../sketch/B1_ref.jpeg) |
| B2 | **極端に長く細い**（= 既存 `long` 相当） | 幾何は通るが Reach では過剰。学習が要る側の例 | ![B2 参照画](../sketch/B2_ref.jpeg) |
...
## 画像ファイルの置き場と命名（2026-09-01）

**`docs/sketch/` に、1枚につき3段階ぶんを残す。** どの段階の画像かがファイル名で分かるようにする。

| 命名 | 中身 |
|---|---|
| `<名前>_ref.*` | **参照画**。付録プロンプトで Gemini が生成したもの。模写の手本 |
| `<名前>_hand.*` | **手描き**。参照画を人が紙に模写したもの。**これが実験の入力** |
| `<名前>_m1.*` | **M1 出力**。手描きを Gemini が設計図スタイルへ整形したもの。Tripo3D へ渡す |

以降 `_glb` / `_xml` は増やさない。GLB と XML は `data/e2e_<名前>/` と
`assets/mujoco_envs/e2e_<名前>.xml` に入る。
...
|---|---|---|
| ![A1 参照画](../sketch/A1_ref.jpeg) | ![A1 手描き](../sketch/A1_hand.webp) | ![A1 M1出力](../sketch/A1_m1.jpeg) |
