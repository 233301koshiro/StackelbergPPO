...
> **状態（2026-08-31）**: **設計のみ完了。手描きはまだ描いていない。**
...
| # | 形 | 狙い |
|---|---|---|
| B1 | **短くずんぐり**（台座の高さに対しリンクが半分程度） | 第1層が学習ゼロ・数秒で「届きません、N 倍に伸ばしてください」と返すはず。**発表での実演に最適** |
| B2 | **極端に長く細い**（台座の高さに対しリンクが3倍程度） | 幾何は通るが Reach では過剰。学習が要る側の例 |
