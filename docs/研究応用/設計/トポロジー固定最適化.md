...
> **位置づけの更新（2026-08-06）**: 本ファイルは `fix_skeleton` を「通常モードに対する
> オプション」として説明しているが、**本研究では 2026-07 以降すべての実験が `fix_skeleton=true` で
> 走っており、これが本線である**（トポロジーはスケッチが決めるという役割分担のため。
> 第3章 3.11、第6章 6.4.1）。`config.yaml` の既定値が `false` である点は今も正しい。
> 本文中の run 名（`my_robot`・`rrbot_arm_attr_only` 等）は当時の例で、現在は存在しない。
> 実際に使われている起動形は [実験系譜.md](../台帳/実験系譜.md) の各段か `scripts/launch_*.sh` を見ること。
...
nohup env USE_CHOREONOID=1 OMP_NUM_THREADS=1 /choreonoid_ws/install/bin/choreonoid \
  ...
  fix_skeleton=true hydra.run.dir=single_run/<run名> \
  > single_run/<run名>/stdout.log 2>&1 &
```
...
初期リンク長・半径を推定
  ↓ scripts/mesh_to_xml/pipeline.py
    ...
    詳細設計 → docs/研究応用/設計/メッシュXMLパイプライン.md
MuJoCo XML（命名規則に準拠）
  ↓
