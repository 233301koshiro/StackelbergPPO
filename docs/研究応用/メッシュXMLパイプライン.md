...
## 8-3. v2/v2b/v2c と v3 の違いのまとめ（2026-07-31、Q&Aから追記）

同じ元メッシュ（colorful2 GLB）から生成しているのに、v2系（3関節・水平）とv3（4関節・縦型）で
**関節数まで変わる**理由がまとまった記述としてどこにもなかったため、整理して記録する。

### v2/v2b/v2c: 「固定台座+水平正規化」変換

§8-2 の実行コマンド（`FIXED_BASE=1` + `RANGES="-60 60 -90 90 -45 45"`）が示す通り、v2生成時は次の2つの変換を行っている。
...
2. **軸の水平正規化**: 残る3セグメント（upper_arm/forearm/hand）の関節軸を、元の向き（本来は肩=ヨー・肘/手首=ピッチの混在）
   に関わらず**すべてZ軸（ヨー）に統一**し、腕全体をX-Y水平面内に収める（`--task-axis`、§7）。
...
v3は`FIXED_BASE=1`を使わず、台座も含めた元のURDFの関節構成をそのまま保持する。台座自身がヨー軸として関節化され、
残り3関節（肩・肘・手首）もピッチ軸のまま変更しない。結果:
...
v2→v3は「初期姿勢だけ変えた」のではなく、**メッシュ→XML変換時の関節抽出・軸正規化の方針そのものが違う**。
同じ物理メッシュから、(a) 台座を固定して残りを水平面に押し込めたもの（v2系）と、
(b) 台座ごと関節化して元の軸構成を保ったもの（v3）という、別の設計判断で生成された2系統である。
