...
## MuJoCo vs Choreonoid：形態最適化研究での使い分け考察（2026-06-19）

rrbot_arm の Choreonoid 学習（epoch 40 以降）と MuJoCo 並行学習の比較から得られた知見をまとめる。

### 数値安定性の違い

Choreonoid（AISTSimulator）と MuJoCo は物理積分の方式が根本的に異なる。

| 項目 | MuJoCo | Choreonoid |
|---|---|---|
| 積分方式 | 半陰的オイラー（soft constraint） | LCP（厳密な剛体接触） |
| 数値安定性 | 高い（逃げ道あり） | 不安定になりやすい |
| サンプリング速度 | ~5s/epoch | ~16s/epoch（3倍遅い） |
| 更新速度 | ~150s/epoch | ~67s/epoch（速い） |
| `train_R=nan` 発生 | ゼロ回 | 複数回（epoch 21, 37 等） |

Choreonoid では `get_body_com()` が物理不安定時に NaN を返し、これが報酬→訓練バッチ→勾配更新と伝播して policy が破損するケースが確認された（`train_R=nan` → 数 epoch 後に `exec_R_eps` 急落）。MuJoCo では同現象は一度も起きていない。

### 形態最適化研究における適切なエンジン選択

Choreonoid の優位性は「実機に近い物理精度」にある。ただしこれが意味を持つのは以下の条件が揃う場合に限られる：

- 最終的に実機転送を想定している
- 探索する形状が実際に製造可能なもの
- sim-to-real gap の定量化が研究目的に含まれる

**本研究のように「現実に存在しない不安定な形状を探索する」場合**、この優位性は活きない。存在しないロボットの sim-to-real gap は議論する意味がなく、探索フェーズでは安定性と速度の方が価値が高い。

### 推奨する使い分け

| フェーズ | 推奨エンジン | 理由 |
|---|---|---|
| 形状探索・強化学習 | **MuJoCo** | 速い・安定・NaN が出ない |
| 候補形状の精度検証 | **Choreonoid** | 製造可能な形状の物理精度確認 |
| 実機転送前の最終確認 | **Choreonoid** | sim-to-real ギャップ計測 |
