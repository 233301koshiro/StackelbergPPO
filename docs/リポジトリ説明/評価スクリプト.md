...
### 画面録画（`record_cnoid_viewer.sh`、2026-07-31追加）

`eval_cnoid_viewer.py`の再生を自動で画面録画したい場合は`scripts/record_cnoid_viewer.sh`を使う。

```bash
VIEWER_RESTORE_DIR=single_run/tripo_arm_v2c_pusher VIEWER_EPOCH=best VIEWER_EPISODES=3 \
  bash scripts/record_cnoid_viewer.sh /path/to/output.mp4
```

Choreonoidを起動し、`xdotool`でウィンドウ出現を検出してからそのウィンドウ領域だけを`ffmpeg`(x11grab)で録画開始。Choreonoidプロセスが終了（全エピソード再生完了）すると自動的に録画を止めてmp4を確定する。`ffmpeg`/`xdotool`が未導入なら自動でaptインストールする。

**負荷について**: x11grabは無圧縮のraw pixelを毎フレームXサーバから取得するため重い（フル解像度4384×2318・25fpsだと1フレーム約40MB相当）。学習プロセスと並行して動かすとCPUを取り合い、実際に画面全体が重くなる現象が確認された（2026-07-31）。そのため既定値を軽量化済み: `RECORD_FPS`（既定12、元は25）・`RECORD_SCALE_WIDTH`（既定1600pxへ縮小、0で無効化可）・エンコードを`libx264 -preset ultrafast -crf 28`に変更。テストでは96秒/6.5MB（旧設定）→59秒/1.4MBまで軽量化し、画質も視認には十分だった。両方とも環境変数で上書き可能:

```bash
RECORD_FPS=25 RECORD_SCALE_WIDTH=0 VIEWER_RESTORE_DIR=... bash scripts/record_cnoid_viewer.sh out.mp4  # フル画質に戻す例
```

**実装時に分かった制約**:
- **ChoreonoidのMovieRecorderはPythonに公開されていない**: C++側（`src/Base/MovieRecorder.h`）には存在するが、`src/Base/pybind11/PyBaseModule.cpp`にMovie/Record関連の登録が一切なく、Pythonスクリプトから直接制御することはできない（2026-07-31確認）。そのため本スクリプトはOS側のffmpeg x11grabで外部から録画している
- **マルチモニタ環境での座標系のズレに注意**: この開発環境は仮想デスクトップが2枚のモニタ横並び（各4384×2466、合計8768×2466）になっており、`xdotool getdisplaygeometry`は**モニタ単位のサイズ**（4384×2466）しか返さない。一方x11grabが実際に見るroot windowは合計サイズ（8768×2466）なので、2枚目のモニタ（x=4384〜）にウィンドウがあるとクランプ計算を誤り録画範囲が画面外にはみ出してffmpegが起動失敗する。正しい全体サイズは`xdpyinfo | grep dimensions`から取得する必要がある（`record_cnoid_viewer.sh`内で対応済み）
