...
> **補足（2026-08-06）**: ファイル構成の説明は現在も有効（記載されている .py はすべて実在を確認済み）。
> コマンド例の run 名は **`<run名>` というプレースホルダ**にしてある（2026-08-06 に、存在しない
> 実 run 名 `pusher_cnoid` から置換。コピペして失敗するのを防ぐためと、名前が変わっても陳腐化しないため）。
> 実際の run 名は `single_run/` を見るか [実験一覧_詳細.md](../実験一覧_詳細.md) を参照。
> `scripts/` のどれが再利用可能でどれが一度きりかは [scripts/README.md](../../scripts/README.md) にある。
...
```
single_run/<run名>/          ← hydra.run.dir で指定した学習ディレクトリ
│
...
```python
import pickle
cp = pickle.load(open('single_run/<run名>/models/best.p', 'rb'))
# または
import torch
cp = torch.load('single_run/<run名>/models/best.p', weights_only=False)
```
...
```bash
python design_opt/eval.py --restore_dir single_run/<run名> --save_video
```
...
```bash
tensorboard --logdir single_run/<run名>/tb
```
